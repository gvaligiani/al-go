...
func TestDeepSet(t *testing.T) {
	...
	assertDeepEqual(t, set.NewDeep(&Item{Value: 12}), odds, "wrong odds")
	...
	require.Equal(t, true, updated, "wrong updated!")
	assertDeepEqual(t, set.NewDeep(&Item{Value: 11}), evens, "wrong evens")

	// check source of copy

	assertDeepEqual(t, set.NewDeep(&Item{Value: 12}, &Item{Value: 11}), s, "source of copy has been modified")
